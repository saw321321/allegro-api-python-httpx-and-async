...
class AsyncCategoriesResource(AsyncBaseResource):
    """Resource for working with categories."""
    
    async def list(self, parent_id: str = None) -> Dict[str, Any]:
        """
        List categories.
        ...
        return await self.client.get("/sale/categories", params=params)
    ...
    async def get(self, category_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Category details
        """
        return await self.client.get(f"/sale/categories/{category_id}")
    
    async def get_parameters(self, category_id: str) -> Dict[str, Any]:
        """
        Get category parameters.
        ...
        Returns:
            Category parameters
        """
        return await self.client.get(f"/sale/categories/{category_id}/parameters")
    ...
    async def get_tree(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            Category tree
        """
        async def fetch_subcategories(parent_id: Optional[str] = None) -> List[Dict[str, Any]]: #Tu nie wiem czy nie dodałem za dużo awaitów
            """Recursively fetch subcategories."""
            response = await self.list(parent_id)
            categories = response.get("categories", [])
        ...
        return await fetch_subcategories()
    
    async def search(self, phrase: str, parent_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Search categories by phrase.
        ...
        async def search_in_tree(categories: List[Dict[str, Any]], search_phrase: str) -> List[Dict[str, Any]]:
            """Search in category tree."""
            results = []
        ...
        # Get category tree starting from parent
        tree = await self.get_tree() if not parent_id else fetch_subcategories(parent_id)  #brak metody fetch_subcategories (to samo jest w nie async wersji)
        
        return await search_in_tree(tree, phrase)
    
    async def get_most_popular(self, limit: int = 20) -> List[Dict[str, Any]]:
        """
        Get most popular categories.
    ...
    async def suggest(self, name: str) -> List[Dict[str, Any]]:
        """
        Get category suggestions based on offer name.
        ...
        try:
            params = {"name": name}
            response = await self.client.get("/sale/category-suggestions", params=params)
            return response.get("categories", [])
        except:
            # Fallback to search
            return await self.search(name)[:10]